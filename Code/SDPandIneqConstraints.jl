# Code used to generate SDP problems with a quadratic objective and a system of inequality constraints save in Testproblems/SDPandEqConstraints with an accurante
# solution from MOSEK
...
# 2. Half the elements in s belong to inequality constraints, other half to one sdp cone, s=[s1;s2], with s1 in R+-cone and s2 in psd-cone
# 3. Create random solution vector xtrue and strue with s1 in R+, and mat(s2) in psd-cone
...
dirPath = "../DataFiles/Julia/SDPandIneqConstraints/"
...
nn = 30
 ...
 for iii =1:1:nn
  ...
  K = Cone(0,m1,[],[r^2])
  # setOFF = OSSDPSettings(rho=0.1,sigma=1e-6,alpha=1.6,max_iter=2000,verbose=false,checkTermination=1,scaling = 0,eps_abs = 1e-7,eps_rel=1e-7,adaptive_rho=true)
 ...
 # @test norm(solTrue-res1.x,Inf) < 1e-2
  ...
  fn = "SDPandIneqConstraints"*nr*".jld"
  problemType = "Random SDP with inequality constraints"
  problemName = "SDPandIneqConstraints"*nr
